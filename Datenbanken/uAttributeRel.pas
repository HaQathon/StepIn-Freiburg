...
//    procedure addToDatabase(Attribut: TAttribut);
    ...
    function getTaskID(AttributID:Integer): TStringlist;
...
function TAttributRelDatenbank.getTaskID(AttributID:Integer): TStringlist;
   var
    query: TUniQuery;
    helpStr : string;
   begin
    ...
    helpStr := IntToStr(AttributID);
    try
      query.Connection := fSQLConnection;
      query.SQL.Text := 'SELECT ' + cTASK_ID + ' FROM ' + cTABLE_NAME + ' WHERE ' + cATTRIBUT_ID + ' = ' + IntToStr(AttributID);
      query.Execute;
      query.Open;
      while not query.Eof do
         begin
          Result.Add(query.FieldByName(cTASK_ID).AsString);
          query.Next;
         end;
    finally
      query.Free;
...
initialization
  AttributRelDatenbank := TAttributRelDatenbank.Create(TPath.Combine(getPrivateDataPath(), CONTENT_URI_ATTRIBUTREL));
