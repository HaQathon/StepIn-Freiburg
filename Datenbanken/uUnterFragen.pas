...
const
  AUTHORITY = '';
  BASE_PATH_UNTERFRAGEN = 'UnterFragen';
  CONTENT_URI_UNTERFRAGEN = BASE_PATH_UNTERFRAGEN  ;
...
procedure TUnterFrage.fillFields(query: TUniQuery);
   begin
    ...
    fragen       := query.FieldByName(unterFragenDatenbank.cFrage).AsString;
...
procedure TUnterFrage.fillParamsOfQuery(query: TUniQuery);
   begin
    ...
    query.ParamByName(unterFragenDatenbank.cFrage).AsString    :=  Fragen;
   end;
...
initialization
  unterFragenDatenbank := TUnterFragenDatenbank.Create(TPath.Combine(getPrivateDataPath(), CONTENT_URI_UNTERFRAGEN ));
