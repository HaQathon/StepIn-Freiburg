...
//    procedure addToDatabase(Task: TTask);
    ...
    function getTaskname (TaskID : Integer) : TStringlist;

    function getTableName: String;override;
    procedure getColumnNames(columnList: TList<String>);override;

    const cTABLE_NAME = 'Task';
    const cID = 'id';
...
function TTaskDatenbank.getTaskname (TaskID : Integer) : TStringlist;
   var
    query: TUniQuery;
   begin
      ...
      try
        ...
        query.SQL.Text := 'SELECT ' + cTASK_NAME + ' FROM ' +cTABLE_NAME +' WHERE ' + cID + ' = ' + IntToStr(TaskID);
        query.Execute;
        query.Open;
        while not query.Eof do
           begin
            Result.Add(query.FieldByName(cTASK_NAME).AsString);
            query.Next;
           end;
      finally
        query.Free;
...
initialization
  TaskDatenbank := TTaskDatenbank.Create(TPath.Combine(getPrivateDataPath(), CONTENT_URI_TASK));
